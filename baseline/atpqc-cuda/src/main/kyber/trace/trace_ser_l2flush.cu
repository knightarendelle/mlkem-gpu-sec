//
// trace_ser_l2flush.cu
// Serialized Kyber decapsulation timing with inter-kernel L2 flush.
//
// Identical to trace_serialized.cu except that every kernel launch is
// followed by cudaCtxResetPersistingL2Cache() in addition to the
// cudaDeviceSynchronize() already present in LAUNCH_TIMED.
//
// Hypothesis: if L2 dirty-line residue (amplified by Ada DCC compression)
// causes per-class timing differences, flushing the persisting L2 partition
// between every kernel should reduce or eliminate the TVLA signal.
//
// Expected outcome: if DCC dirty-line context is the sole mechanism,
// leaking kernels (7/27 in baseline) should drop to 0/27.
// If leakage persists, the effect is intrinsic to each kernel's own I/O
// pattern (not upstream context) and requires a different mitigation.
//
// Output: identical CSV format to trace_serialized.cu (trace_id,kernel,us).
//
// Usage:
//   echo "1 4 4 4 4 100000 0" | ./target/trace_ser_l2flush_kyber1024.out \
//     > experiments/traces/l2flush/kyber1024_l2flush_class0_n100000.csv
// Build:
//   make -C baseline/atpqc-cuda trace_ser_l2flush_kyber1024
//
...
#undef  CCC
static inline cudaError_t lf_check(cudaError_t e,
                                    const char* call, const char* file, int line) {
    if (e != cudaSuccess) {
        std::fprintf(stderr, "[L2FLUSH CUDA ERROR] %s:%d\n  call: %s\n  error: %s\n",
...
static inline CUresult lf_check(CUresult e,
                                  const char* call, const char* file, int line) {
    if (e != CUDA_SUCCESS) {
...
#define CCC(call) lf_check((call), #call, __FILE__, __LINE__)

namespace atpqc_cuda::kyber::trace_ser_l2flush {

using rng_type = rng::std_random_device;
using variant  = variants::KYBER_VARIANT;
constexpr variant variant_v;

// ── Per-kernel timed launch with L2 flush ────────────────────────────────
// Identical to LAUNCH_TIMED in trace_serialized.cu except for the
// cudaCtxResetPersistingL2Cache() call after DeviceSync.
#define LAUNCH_TIMED_FLUSH(label, obj, stream, ev_a, ev_b, record, tid, ...)  \
  do {                                                                          \
...
void trace_serialized_l2flush() {
  ...
  auto run_one = [&](unsigned trace_i, bool record) {
    ...
    LAUNCH_TIMED_FLUSH("decompress_u", decompressu, stream, ev_start, ev_stop,
                       record, trace_i,
                       bp_dec, ct_d.get_ptr(), ct_d.get_pitch());

    LAUNCH_TIMED_FLUSH("decompress_v", decompressv, stream, ev_start, ev_stop,
                       record, trace_i,
                       v_dec,
                       ct_d.get_ptr() + params::polyveccompressedbytes<variant>,
                       ct_d.get_pitch());

    LAUNCH_TIMED_FLUSH("decode_s", decodes, stream, ev_start, ev_stop,
                       record, trace_i,
                       skpv_dec, sk_d.get_ptr(), sk_d.get_pitch());

    LAUNCH_TIMED_FLUSH("fwdntt_u", fwdnttvec_u, stream, ev_start, ev_stop,
                       record, trace_i,
                       bp_dec);

    LAUNCH_TIMED_FLUSH("s_times_u", stimesu, stream, ev_start, ev_stop,
                       record, trace_i,
                       mp_dec, skpv_dec, bp_dec);

    LAUNCH_TIMED_FLUSH("intt_su", intt_su, stream, ev_start, ev_stop,
                       record, trace_i,
                       mp_dec);

    LAUNCH_TIMED_FLUSH("v_minus_su", psub, stream, ev_start, ev_stop,
                       record, trace_i,
                       mp_dec, v_dec, mp_dec);

    LAUNCH_TIMED_FLUSH("poly_tomsg", tomsg, stream, ev_start, ev_stop,
                       record, trace_i,
    ...
    LAUNCH_TIMED_FLUSH("hash_coin", dec_hash_coin, stream, ev_start, ev_stop,
                       record, trace_i,
                       kr_ptr, kr_pitch, buf_ptr, buf_pitch, 2 * params::symbytes);

    LAUNCH_TIMED_FLUSH("decode_t", decodet, stream, ev_start, ev_stop,
                       record, trace_i,
                       pkpv_enc, pk_in_sk, sk_d.get_pitch());

    LAUNCH_TIMED_FLUSH("poly_frommsg", frommsg, stream, ev_start, ev_stop,
                       record, trace_i,
                       k_enc, buf_ptr, buf_pitch);

    LAUNCH_TIMED_FLUSH("generate_at", generate_at, stream, ev_start, ev_stop,
                       record, trace_i,
                       at_enc,
                       pk_in_sk + params::polyvecbytes<variant>,
                       sk_d.get_pitch());

    LAUNCH_TIMED_FLUSH("generate_r", generate_r, stream, ev_start, ev_stop,
                       record, trace_i,
                       sp_enc, kr_ptr + params::symbytes, kr_pitch,
                       static_cast<std::uint8_t>(0));

    LAUNCH_TIMED_FLUSH("generate_e1", generate_e1, stream, ev_start, ev_stop,
                       record, trace_i,
                       ep_enc, kr_ptr + params::symbytes, kr_pitch,
                       static_cast<std::uint8_t>(params::k<variant>));

    LAUNCH_TIMED_FLUSH("generate_e2", generate_e2, stream, ev_start, ev_stop,
                       record, trace_i,
                       epp_enc, kr_ptr + params::symbytes, kr_pitch,
                       static_cast<std::uint8_t>(params::k<variant> * 2));

    LAUNCH_TIMED_FLUSH("fwdntt_r", fwdnttvec_r, stream, ev_start, ev_stop,
                       record, trace_i,
                       sp_enc);

    LAUNCH_TIMED_FLUSH("a_times_r", mtv, stream, ev_start, ev_stop,
                       record, trace_i,
                       bp_enc, at_enc, sp_enc);

    LAUNCH_TIMED_FLUSH("t_times_r", ttimesr, stream, ev_start, ev_stop,
                       record, trace_i,
                       v_enc, pkpv_enc, sp_enc);

    LAUNCH_TIMED_FLUSH("intt_ar", intt_ar, stream, ev_start, ev_stop,
                       record, trace_i,
                       bp_enc);

    LAUNCH_TIMED_FLUSH("intt_tr", intt_tr, stream, ev_start, ev_stop,
                       record, trace_i,
                       v_enc);

    LAUNCH_TIMED_FLUSH("ar_plus_e1", vpv, stream, ev_start, ev_stop,
                       record, trace_i,
                       bp_enc, bp_enc, ep_enc);

    LAUNCH_TIMED_FLUSH("tr_plus_e2_plus_m", padd3, stream, ev_start, ev_stop,
                       record, trace_i,
                       v_enc, v_enc, epp_enc, k_enc);

    LAUNCH_TIMED_FLUSH("compress_u", compressu, stream, ev_start, ev_stop,
                       record, trace_i,
                       cmp_ptr, cmp_pitch, bp_enc);

    LAUNCH_TIMED_FLUSH("compress_v", compressv, stream, ev_start, ev_stop,
                       record, trace_i,
                       cmp_ptr + params::polyveccompressedbytes<variant>,
                       cmp_pitch, v_enc);

    LAUNCH_TIMED_FLUSH("hash_ct", dec_hash_ct, stream, ev_start, ev_stop,
                       record, trace_i,
                       kr_ptr + params::symbytes, kr_pitch,
                       ct_d.get_ptr(), ct_d.get_pitch(),
                       params::ciphertextbytes<variant>);

    LAUNCH_TIMED_FLUSH("verify_cmov", dec_verify_cmov, stream, ev_start, ev_stop,
                       record, trace_i,
                       kr_ptr, kr_pitch,
    ...
    LAUNCH_TIMED_FLUSH("kdf", dec_kdf, stream, ev_start, ev_stop,
                       record, trace_i,
...
int main() {
...

  {
    ...
    atpqc_cuda::kyber::trace_ser_l2flush::trace_serialized_l2flush();
