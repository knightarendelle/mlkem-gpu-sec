//
// trace_ser_l2thrash_interleaved.cu
// Interleaved valid-vs-random TVLA with full L2 eviction between every kernel.
//
// Fixes the sequential-collection artifact in trace_ser_l2thrash.cu by running
// both classes within a single binary, strictly alternating per iteration.
// Both classes experience the same GPU thermal and clock state throughout.
//
...
// Both classes use the same keypair K0 for decapsulation.
// L2 is evicted (2× L2-sized cudaMemsetAsync) between every kernel.
//
...
//
// Usage:
//   echo "1 4 4 4 4 100000" | ./target/trace_ser_l2thrash_interleaved_kyber1024.out \
//     experiments/traces/l2thrash/kyber1024_interleaved_class0_n100000.csv \
//     experiments/traces/l2thrash/kyber1024_interleaved_class1_n100000.csv
// Build:
//   make -C baseline/atpqc-cuda trace_ser_l2thrash_interleaved
//
...
#undef  CCC
static inline cudaError_t lti_check(cudaError_t e,
                                     const char* call, const char* file, int line) {
    if (e != cudaSuccess) {
        std::fprintf(stderr, "[L2THRASH_IL CUDA ERROR] %s:%d\n  call: %s\n  error: %s\n",
...
static inline CUresult lti_check(CUresult e,
                                  const char* call, const char* file, int line) {
    if (e != CUDA_SUCCESS) {
...
#define CCC(call) lti_check((call), #call, __FILE__, __LINE__)

namespace atpqc_cuda::kyber::trace_ser_l2thrash_interleaved {
...
// L2 flush:       cudaMemsetAsync(thrash_buf, l2_bytes) + DeviceSync AFTER timing
// total_us:       accumulated from enclosing run_one scope
// fout:           only written when record=true
//
#define LAUNCH_TIMED_THRASH(label, obj, stream, ev_a, ev_b, record, fout, tid, ...) \
  do {                                                                          \
...
void run(int argc, char** argv) {
  ...
  if (!out0 || !out1) {
    fprintf(stderr, "[l2thrash_il] failed to open output files: %s %s\n",
            argv[1], argv[2]);
    std::abort();
  }
  ...
  fprintf(stderr,
          "[l2thrash_il] %s  ninputs=%u  ntraces_per_class=%u\n"
          "[l2thrash_il] L2 cache: %d bytes (%.1f MB); thrash buffer: %.1f MB\n",
  ...
  // ── Generate K0 ───────────────────────────────────────────────────────────
  fprintf(stderr, "[l2thrash_il] generating K0...\n");
  {
  ...
  fprintf(stderr, "[l2thrash_il] K0 generated.\n");
  ...
  // Upload sk to device (already there from keypair, but sk0_h is pinned copy)
  ...
  // ── Verify K0 enc+dec produce matching shared secrets ─────────────────────
  {
    do_enc0();
    ...
    // Quick single dec via a throw-away lambda (record=false, no fout needed)
    cuda_resource::stream verify_stream(cudaStreamNonBlocking);
    cudaEvent_t ve_a, ve_b;
    CCC(cudaEventCreate(&ve_a)); CCC(cudaEventCreate(&ve_b));

    const std::uint8_t* pk_in_sk_v =
        sk0_d.get_ptr() + params::indcpa_secretkeybytes<variant>;
    const std::uint8_t* z_in_sk_v =
    ...
    // Inline dec (no thrash, no timing needed — just correctness check)
    ...
    fprintf(stderr, "[l2thrash_il] K0 enc/dec shared secret match: %s\n",
            match ? "YES" : "NO");
    if (!match) {
      fprintf(stderr, "[l2thrash_il] ABORT: enc/dec mismatch — check key setup\n");
      std::abort();
    }
    std::fflush(stderr);
    CCC(cudaEventDestroy(ve_a)); CCC(cudaEventDestroy(ve_b));
  }

  // ── Convenience pointers for the timed dec loop ───────────────────────────
  ...
  // ── Timed dec: one iteration, ct already loaded into ct_d ────────────────
  ...
  // ── Warmup: 50 interleaved iterations (each has 28 L2 thrashes) ──────────
  fprintf(stderr, "[l2thrash_il warmup] 50 iterations (interleaved)...\n");
  for (unsigned w = 0; w < 50; w++) {
    if (w % 2 == 0) { do_enc0();    run_one(0, false, nullptr); }
    else             { do_rand_ct(); run_one(0, false, nullptr); }
  }
  fprintf(stderr, "[l2thrash_il warmup] done\n");
  std::fflush(stderr);

  // ── CSV headers ───────────────────────────────────────────────────────────
  auto write_header = [&](FILE* f, int cls) {
    fprintf(f, "# mlkem-gpu-sec per-kernel serialized timing traces (L2 thrash, interleaved)\n");
    fprintf(f, "# variant: %s\n", variant_name);
    fprintf(f, "# class: %d (%s)\n", cls, cls == 0 ? "valid ct (fresh enc per trace)" : "random bytes (fresh per trace)");
    fprintf(f, "# n_traces: %u\n", ntraces_per_class);
    fprintf(f, "# mitigation: full L2 eviction (2x L2-sized cudaMemset) after every kernel\n");
    fprintf(f, "# l2_size_bytes: %d\n", l2_bytes_int);
    fprintf(f, "# thrash_buf_bytes: %zu\n", l2_bytes);
  ...
  fprintf(stderr,
          "[l2thrash_il] done — class0: %u traces  class1: %u traces\n",
          cnt0, cnt1);
  std::fflush(stderr);
...
int main(int argc, char** argv) {
...

  {
    ...
    atpqc_cuda::kyber::trace_ser_l2thrash_interleaved::run(argc, argv);
